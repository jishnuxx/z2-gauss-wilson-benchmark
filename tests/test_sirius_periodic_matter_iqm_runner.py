...
def sirius_manifest(tmp_path: Path) -> dict:
    circuit_records = []
    paths = []
...
def test_sirius_manifest_accepts_reviewed_six_circuit_scan(tmp_path):
    manifest = sirius_manifest(tmp_path)
    paths = validate_sirius_scan_submission_manifest(
        manifest,
        root=tmp_path,
        expected_quantum_computer="sirius",
        expected_shots=5000,
    )
    assert len(paths) == 6


def test_sirius_manifest_accepts_explicit_repeat2_batch(tmp_path):
    manifest = sirius_manifest(tmp_path)
...
def test_sirius_manifest_rejects_changed_circuit_order(tmp_path):
    manifest = sirius_manifest(tmp_path)
    manifest["circuits"][0], manifest["circuits"][1] = (
        manifest["circuits"][1],
        manifest["circuits"][0],
    )
    # Recompute the generic identity so this test reaches the independent,
    # Sirius-specific ordering guard.
    manifest["candidate_id"] = candidate_id(
        {
            "batch": manifest["batch"],
            "server_url": manifest["server_url"],
    ...
    with pytest.raises(PermissionError, match="order"):
        validate_sirius_scan_submission_manifest(manifest, root=tmp_path)


def test_sirius_manifest_rejects_changed_mapping(tmp_path):
    manifest = sirius_manifest(tmp_path)
    manifest["circuits"][0]["logical_to_physical_indices"][0] = 0
    with pytest.raises(PermissionError, match="mapping"):
        validate_sirius_scan_submission_manifest(manifest, root=tmp_path)


def test_sirius_scan_submission_requires_two_consent_signals(tmp_path):
    candidate = sirius_manifest(tmp_path)["candidate_id"]
    require_explicit_sirius_scan_hardware_consent(
        submit=False,
        environment={},
    ...
    with pytest.raises(PermissionError, match="confirm-candidate"):
        require_explicit_sirius_scan_hardware_consent(
            submit=True,
            environment={"Z2LGT_ALLOW_SIRIUS_SCAN_HARDWARE": "YES"},
            candidate_id=candidate,
            confirmation="wrong",
        )
    require_explicit_sirius_scan_hardware_consent(
