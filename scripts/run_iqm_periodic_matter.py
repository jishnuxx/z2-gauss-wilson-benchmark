#!/usr/bin/env python3
"""Run the reduced periodic matter-readout IQM candidate; dry-run by default."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=Path("results/processed/periodic_iqm_matter_readout_5000.csv"),
    )
    ...
    qpy_paths = validate_periodic_matter_submission_manifest(
        manifest,
    ...
    report: dict[str, object] = {
        "schema_version": 1,
        "mode": "iqm-periodic-matter-readout",
        "candidate_id": manifest["candidate_id"],
    ...
    if not args.submit:
        ...
        print("Reduced periodic matter IQM runner dry-run")
    ...
    try:
        ...
        write_json(args.output_json, report)
        print(f"Submitted reduced periodic matter job: {job_id}")
        ...
        report.update(
            {
                ...
                "measured_raw_imbalance_separation": abs(
                    plus["analysis"]["imbalance"] - minus["analysis"]["imbalance"]
                ),
        ...
        print("Reduced periodic matter result completed")
        for record in records:
            analysis = record["analysis"]
            print(
                f"  {record['case']}: O_LR={analysis['imbalance']:.6f} +/- "
