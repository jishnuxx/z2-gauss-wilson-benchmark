#!/usr/bin/env python3
r"""Freeze the reduced two-sector periodic matter-readout IQM candidate.

This is the depth-reduced dynamics candidate selected by
``scripts/run_periodic_depth_reduction_audit.py``.  It measures only the four
matter occupations for \(O_{\rm LR}\); Gauss/Wilson certification remains a
separate diagnostic layer.

The script validates and freezes artifacts only.  It never calls
``backend.run``.
"""
...
from z2lgt.periodic_matter_iqm_runner import MATTER_BATCH_PREFIX
...
def main() -> None:
    ...
    parser.add_argument("--max-native-depth", type=int, default=135)
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("results/iqm/emerald_periodic_matter_candidate_5000"),
    )
    ...
    for (sector, sign), source, circuit in zip(SECTORS, sources, compiled, strict=True):
        mapping = [
        ...
        qpy_path = circuit_dir / f"periodic_matter_{sector}.qpy"
        qasm_path = circuit_dir / f"periodic_matter_{sector}.qasm"
        ...
        records.append(
            {
                ...
                "measurement_count": int(circuit.count_ops().get("measure", 0)),
                "operations": {
                    str(name): int(count)
    ...
    hardware_checks = {
        ...
        "matter_only_measurements": all(
            int(record["measurement_count"]) == 4 for record in records
        ),
    }
    ...
    batch = f"{MATTER_BATCH_PREFIX}t{args.time:g}-dt{args.dt:g}"
    identity = {
        "batch": batch,
        "server_url": args.url,
    ...
    manifest = {
        ...
        **identity,
        ...
        "circuits": records,
        "review_note": (
            "Reduced periodic hardware dynamics candidate.  It measures matter "
            "only for O_LR; use separate Gauss/Wilson diagnostics for sector "
            "certification."
        ),
    }
    ...
    print(f"Frozen reduced periodic matter candidate: {manifest_path}")
