#!/usr/bin/env python3
r"""Freeze the six-circuit Sirius periodic-matter benchmark candidate.

The benchmark contains three two-step product-formula points,
``(t, dt) = (0.6, 0.3), (0.8, 0.4), (1.0, 0.5)``, in both Wilson
sectors.  The logical-to-physical placement is fixed across all six circuits.
...
manifest.  It never calls ``backend.run`` and therefore does not submit a job
or consume hardware credits.
"""
...
# Logical q0,...,q7 -> Sirius component indices.  This placement was selected
# during the read-only Sirius audit and is frozen to make every comparison use
# the same physical qubits.
INITIAL_LAYOUT: tuple[int, ...] = (4, 1, 6, 5, 0, 2, 3, 13)
...
def main() -> None:
    ...
    parser.add_argument("--max-native-depth", type=int, default=180)
    parser.add_argument("--max-native-cz", type=int, default=65)
    parser.add_argument("--max-native-move", type=int, default=90)
    parser.add_argument("--max-r-error", type=float, default=0.003)
    parser.add_argument("--max-cz-error", type=float, default=0.025)
    parser.add_argument("--max-move-error", type=float, default=0.012)
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("results/iqm/sirius_periodic_matter_scan_candidate_5000"),
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    if args.quantum_computer != "sirius":
        raise SystemExit("this target-specific freezer only accepts --quantum-computer sirius")
    if args.shots <= 0:
    ...
    if tests.returncode != 0:
        ...
        raise SystemExit("test gate failed; Sirius candidate was not frozen")
    ...
    for label, source, circuit in zip(labels, sources, compiled, strict=True):
        ...
        cz_loci = calibrated_loci(
            backend, circuit, mapping, "cz", real_target
        )
        move_loci = calibrated_loci(
            backend, circuit, mapping, "move", real_target
        )
    ...
    measured_matter_only = all(
        {
            int(item["component_indices"][0])
            for item in record["measurement_loci"]
        }
        == set(record["logical_to_physical_indices"][:4])
    ...
    maximum_move_error = maximum_error(records, "move_loci")
    maximum_readout_error = maximum_error(records, "measurement_loci")

    hardware_checks = {
        "six_circuit_scan": len(records) == 6,
        "same_fixed_mapping_for_all_circuits": mappings == {INITIAL_LAYOUT},
        ...
        "maximum_native_move_count": maximum_move_count <= args.max_native_move,
        "maximum_r_error": (
        ...
        "maximum_move_error": (
            maximum_move_error is not None
            and maximum_move_error <= args.max_move_error
        ),
    ...
    if not all(hardware_checks.values()):
        ...
        raise SystemExit("Sirius freeze gate failed: " + ", ".join(failed))

    calibration_set_id = str(backend.architecture.calibration_set_id)
    base_batch = "periodic-matter-sirius-two-step-scan-t0.6-0.8-1.0"
    batch = (
        base_batch
    ...
    manifest = {
        ...
        **identity,
        ...
        "fixed_initial_layout_indices": list(INITIAL_LAYOUT),
        "fixed_initial_layout_components": [
            component_name(backend, index) for index in INITIAL_LAYOUT
        ],
        ...
        "hardware_thresholds": {
            ...
            "maximum_native_move_count": args.max_native_move,
            "maximum_r_error": args.max_r_error,
            "maximum_cz_error": args.max_cz_error,
            "maximum_move_error": args.max_move_error,
        ...
        "maximum_move_error": maximum_move_error,
        ...
        "circuits": records,
        "review_note": (
            "Six reduced matter-readout circuits for a two-step Sirius scan. "
            "The circuits do not directly measure Wilson or Gauss operators; "
            "sector certification remains a separate diagnostic layer."
        ),
    }
    ...
    print(f"Frozen Sirius periodic-matter scan: {manifest_path}")
    print(f"Candidate ID: {manifest['candidate_id']}")
    print(f"Calibration set: {calibration_set_id}")
    print(
        "Fixed mapping:",
        list(zip(range(len(INITIAL_LAYOUT)), manifest["fixed_initial_layout_components"])),
    )
    for record in records:
        print(
    ...
    print(
        ...
        f"MOVE={maximum_move_error:.6f} readout={maximum_readout_error:.6f}"
    )
